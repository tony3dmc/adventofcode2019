...
const closestCrossingPoint = (wires) => {
  ...
  wires.forEach((wire, id) => {
    ...
    wire.forEach(op => {
      ...
      length += distance
      wirePositions[id].push({x: position.x, y: position.y, length})
    })
  })
  ...
  //console.log(Math.min(...crossings.map(c => manhatten(c))))
  return Math.min(...crossings.map(c => c.length))
